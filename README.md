...
Tired of watching the LLM fail at even basic tasks, mindlessly repeating obviously wrong actions, and having to use the admin interface to yell at it? Well I've got just the thing for you!
This project seems to simulate Reinforcement Learning through LLM context, doesn't it? Well then, why not borrow another thing from Reinforcement Learning: Reward modelling(kinda)?
Executing autocritic.py alongside auto.py introduces a "critic model", which periodically chimes in and automaticaly generates feedback from a high level. This critic model has additional context that the main model doesn't, it knows what the project is about, what solutions are possible, etc. Based on instructions given by "manual.txt", it automatically inputs its advice to "admin.txt". This is a bit of a hack, and the system is prone to reward hacking, but it does make the whole system more competent with less human oversight.

A full GNU screen session containing both agent model and critic model processes can be created using "fullsession.sh". The exact configuration can be set in "sessionconfig.txt".

Results using autocritic are promising. Even a small model, Qwen3:4b-instruct has been seen passing Gate 2. Currently observed failures seem to mainly be from the agent model ignoring the advice of the critic model rather than the critic model being gamed, which makes this method more compelling. However, small models often cripple themselves by mindlessly deleting core files at the beginning of Gate 0, which means just like traditional RL, the problem of safe exploration is still important.

However, the autocritic is prone to leaking information about the gates, and often cannot guide the agent model whilst witholding information the agent model has not figured out yet. Subtle hinting does not seem to be LLMs' strong suit, as they've been trained to directly suggest solutions, not guide the learning process. So I've decided to treat the autocritic not as an all-knowing pseudo-authority but as a second voice to help the model, and created an option to retract its access to the project details. This is done by setting the reference file name to "" via the option '-r ""'. This is currently the default for sessionconfig.txt.

**Further development: Continuation Mode**

I've already heavily criticized the fact that these models are trained to be a "helpful assistant", and are eventually treated like the Genie. If so, how would a pure text continuation model(created by using the "generate" function and not the "chat" function of Ollama), one without all the scaffolding around it to follow directed orders etc, act in my environment? To test this, I created a new flag: "-q" or "--query" to auto.py. This lets you select different query modes, and right now, there are two: "chat" and "continue". Those names should be self-exaplanatory. I also created a special system prompt that uses example transcripts to get the continuation model started. Let's see how that goes...
