...
However, the autocritic is prone to leaking information about the gates, and often cannot guide the agent model whilst witholding information the gent model has not figured out yet. Subtle hinting does not seem to be LLMs' strong suit, as they've been trained to directly suggest solutions, not guide the learning process. So I've decided to treat the autocritic not as an all-knowing pseudo-authority but as a second voice to help the model, and created an option to retract its access to the project details. This is done by setting the reference file name to "" via the option '-r ""'.
